...
void Maestro::WritePlotFile(
    ...
    const Vector<IntVect> ref_ratio = refRatio();
    ...
    if (!is_small) {
	    ...
	    const int IOP        = ParallelDescriptor::IOProcessorNumber();
	    ParallelDescriptor::ReduceRealMax(deal,IOP);
	    amrex::Print() << "dir time = " << deal << "  seconds" << "\n\n";
            /*if (ParallelDescriptor::MyProc() == ParallelDescriptor::NProcs()-1) {
                ...
                auto f = [=]() {
                    ...
                    WriteGenericPlotfileHeader(HeaderFile, nlevels, boxArrays, varnames,
                                            geom, t_in, step_array, ref_ratio, versionName,
            ...
            amrex::WriteMultiLevelPlotfileHDF5SingleDset(plotfilename, nlevels,
                                mf,
                                varnames, geom,
                                t_in, step_array, ref_ratio,
                                compression,
                                versionName,
                                levelPrefix,
                                mfPrefix);
	    if(ParallelDescriptor::IOProcessor())
        	writeFile(plotfilename);
	    ParallelDescriptor::Barrier();
#endif
    } else {
        int nSmallPlot = 0;
