                   ...
                   const int& plt_step)
{
    ...
    std::string prefix;
    ParmParse pp("amr");
    pp.queryAdd("plot_file", prefix);
    const std::string& pltfile = amrex::Concatenate(prefix,plt_step,8);
    ...
    WriteSingleLevelPlotfileHDF5SingleDset(pltfile, Plt, var_names, geom, time, plt_step, 
                                        compression, "HyperCLaw-V1.1", "Level_",  "Cell");
}
