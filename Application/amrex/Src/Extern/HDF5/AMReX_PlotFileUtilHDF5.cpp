...
#if (defined AMREX_USE_HDF5_ZFP) || (defined AMREX_USE_HDF5_SZ) || (defined AMREX_USE_HDF5_SZ3) || (defined AMREX_USE_HDF5_ZLIB) || (defined AMREX_USE_HDF5_SZADT)
...
// #endif
    ...
    for (int level = 0; level <= finest_level; ++level) {
...
#if (defined AMREX_USE_HDF5_SZ) || (defined AMREX_USE_HDF5_SZ3) || (defined AMREX_USE_HDF5_SZADT)
        double eb = 0.001;
        ...
        if (mode_env == "SZ") {
            ...
            unsigned int* cd_values = NULL;
            unsigned filter_config;
            SZ_errConfigToCdArray(&cd_nelmts, &cd_values, 1, eb, eb, 0, 1024);
            H5Pset_filter(dcpl_id_lev, H5Z_FILTER_SZ, H5Z_FLAG_MANDATORY, cd_nelmts, cd_values);
        }
#endif
...
auto deal0 = amrex::second();
    ...
    H5Fclose(fid);
#endif
    auto  deal = amrex::second() - deal0;
    const int IOProc        = ParallelDescriptor::IOProcessorNumber();
    ParallelDescriptor::ReduceRealMax(deal, IOProc);
    amrex::Print() << "deal time = " << deal << "  seconds" << "\n\n";
// //dcdc-start
