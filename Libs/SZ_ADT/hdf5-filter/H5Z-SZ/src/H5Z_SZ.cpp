...
sz_params* H5Z_SZ_Init_Default()
{
    ...
    confparams_cpr->szMode = SZ_BEST_SPEED;// SZ_BEST_COMPRESSION;
...
static size_t H5Z_filter_sz(unsigned int flags, size_t cd_nelmts, const unsigned int cd_values[], size_t nbytes, size_t* buf_size, void** buf)
{
    ...
    const char* env = getenv("compressionType");
    if (!env || strcmp(env, "2") == 0) {
        confparams_cpr->entropy_type = 2;  
        confparams_cpr->szMode = SZ_BEST_SPEED;
    }
    else if (!env || strcmp(env, "0") == 1) {
        confparams_cpr->entropy_type = 0;  
        confparams_cpr->szMode = SZ_BEST_SPEED;
    }
	...
	else //compression
	{
        ...
        // auto duration = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start);  // 计算时间间隔（单位：纳秒）
        
        // printf("Compression time: %lld ns\n",duration.count());
		//cost_end();
